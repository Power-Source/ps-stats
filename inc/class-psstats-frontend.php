<?php
/**
 * PSStats: PSStats_Frontend class
 *
 ...
 * @since 1.4.0
 */
class PSStats_Frontend extends PSStats {
	...
	public static function track_visit( $is_snippet = false ) {
		...
		if ( self::is_javascript_tracking_enabled() ) {
			...
			if ( isset( $_REQUEST['psstats_target'] ) ) {
				$target = filter_var( wp_unslash( $_REQUEST['psstats_target'] ), FILTER_SANITIZE_URL );
			}
			if ( isset( $_REQUEST['psstats_referrer'] ) ) {
				$referrer = filter_var( wp_unslash( $_REQUEST['psstats_referrer'] ), FILTER_SANITIZE_URL );
			}
		} else {
		 ...
		 */
		do_action( 'psstats__visit_saved', $data, $wpdb->insert_id );
	...
	public static function track_visit_ajax() {
		...
		// Check AJAX referrer.
		if ( PSStats::TRACKING_METHOD_JAVASCRIPT_WITH_NONCE_CHECK === self::$_options['snippet'] ) {
			check_ajax_referer( 'psstats_track' );
		}
	 ...
	 * @version  1.6.3
	 *
	 * @hook     boolean  psstats__skip_tracking
	 *
	...
	private static function _skip_tracking() {
		...
		if ( function_exists( 'apply_filters_deprecated' ) ) {
			apply_filters_deprecated( 'psstats_skip_tracking', array( '' ), '1.5.0', 'psstats__skip_tracking' );
		}
		// Skip tracking via Hook.
		$skip_hook = apply_filters( 'psstats__skip_tracking', null );
		if ( null !== $skip_hook ) {
			return $skip_hook;
		}
	...
	public static function wp_footer() {
		...
		wp_localize_script(
			...
			array(
				...
				'nonce' => wp_create_nonce( 'psstats_track' ),
			)
		);
	}
	...
	public static function amp_analytics_entries( $analytics_entries ) {
		...
		if ( self::is_javascript_tracking_enabled() ) {
			$analytics_entries['psstats'] = array(
				'type'   => '',
	...
	private static function make_amp_config() {
		return array(
			...
			'extraUrlParams' => array(
				...
				'_ajax_nonce'      => wp_create_nonce( 'psstats_track' ),
				'psstats_referrer' => '${documentReferrer}',
				'psstats_target'   => '${canonicalPath}amp/',
			),
			'triggers'       => array(
				'trackPageview' => array(
					'on'      => 'visible',
