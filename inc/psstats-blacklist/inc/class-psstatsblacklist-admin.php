<?php
/**
 * PSStats Filter: PSStatsBlacklist_Admin class
 *
...
/**
 * PSStats Filter admin configuration.
 *
 * @since   1.0.0
 */
class PSStatsBlacklist_Admin extends PSStatsBlacklist {
	...
	public static function init() {
		// Add actions.
		add_action( 'wpmu_new_blog', array( 'PSStatsBlacklist_System', 'install_site' ) );
		add_action( 'delete_blog', array( 'PSStatsBlacklist_System', 'uninstall_site' ) );
		add_filter( 'plugin_row_meta', array( 'PSStatsBlacklist_Admin', 'plugin_meta_link' ), 10, 2 );

		if ( self::$multisite ) {
			add_action( 'network_admin_menu', array( 'PSStatsBlacklist_Admin', 'add_menu_page' ) );
			add_filter(
			...
			add_action( 'admin_menu', array( 'PSStatsBlacklist_Admin', 'add_menu_page' ) );
			add_filter( 'plugin_action_links', array( 'PSStatsBlacklist_Admin', 'plugin_actions_links' ), 10, 2 );
		}
	}
	...
	public static function update_options( $options = null ) {
		if ( isset( $options ) && current_user_can( 'manage_options' ) ) {
			...
			if ( self::$multisite ) {
				update_site_option( 'psstats-blacklist', $options );
			} else {
				update_option( 'psstats-blacklist', $options );
			}
		}
	...
	public static function add_menu_page() {
		$title = __( 'PS-Stats Filter', 'psstats' );
		if ( self::$multisite ) {
			add_submenu_page(
				'settings.php',
				$title,
	...
	/**
	 * Include the PSStats-Blacklist settings page.
	 *
	 * @since 1.0.0
	 */
	public static function settings_page() {
		include CPSTATSBLACKLIST_DIR . '/views/settings-page.php';
	}
	...
	public static function plugin_meta_link( $links, $file ) {
		if ( CPSTATSBLACKLIST_BASE === $file ) {
			$links[] = '<a href="https://github.com/stklcode/psstats-blacklist">GitHub</a>';
		}
	...
	public static function plugin_actions_links( $links, $file ) {
		...
		if ( CPSTATSBLACKLIST_BASE === $file && current_user_can( 'manage_options' ) ) {
			array_unshift(
				$links,
				sprintf( '<a href="%s">%s</a>', esc_attr( add_query_arg( 'page', 'psstats', $base ) ), __( 'Settings', 'psstats' ) )
			);
		}
	...
	public static function cleanup_database() {
		...
		if ( ! empty( $referer_regexp ) || ! empty( $target_regexp ) ) {
			...
			if ( ! empty( $referer_regexp ) ) {
				$wpdb->query(
					$wpdb->prepare(
						"DELETE FROM `$wpdb->psstats` WHERE "
